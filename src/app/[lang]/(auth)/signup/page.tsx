...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (data.needsEmailConfirm) {
      // E-posta onayı gerekiyor — başarı ekranı göster
      setDone(true)
      setLoading(false)
    } else {
      // Otomatik oturum açıldı — brand setup'a yönlendir
    ...
    <main className="min-h-screen flex items-center justify-center px-4 bg-background overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-muted-foreground">Ücretsiz hesap oluştur</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {done ? (
            ...
            >
              ...
              <h2 className="text-lg font-bold text-foreground mb-2">E-postanı kontrol et</h2>
              <p className="text-sm text-muted-foreground leading-relaxed">
                <span className="text-foreground font-medium">{email}</span> adresine bir onay bağlantısı gönderdik.
